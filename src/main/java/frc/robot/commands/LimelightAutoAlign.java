...
public class LimelightAutoAlign extends CommandBase {
  ...
  public void execute() {
    ...
    if (isTarget) {
      ...
      // Set minimum turn speed if turnAlign < threshold and offset to target still
      // exists
      ...
      if ((Math.abs(moveAlign) < Constants.kTargetMoveMinThreshold) && (Math.abs(ty) > Constants.kMinYTargetOffset)) {
        // Set minimum turn speed if turnAlign < threshold and off to target still
        // exists
  ...
  public boolean isFinished() {
    ...
    if (m_limeLight.isTarget() && ((Math.abs(m_limeLight.getTx())) < Constants.kMinXTargetOffset)
        && ((Math.abs(m_limeLight.getTy())) < Constants.kMinYTargetOffset)) {
