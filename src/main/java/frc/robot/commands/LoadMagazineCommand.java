...
public class LoadMagazineCommand extends CommandBase {
  ...
  public void execute() {
    ...
    if (lastState == true && currentState == false) {
      ...
      if (Constants.kDebug) {
        System.out.print("Magazine CMD Falling edge detected- Balls=");
    ...
    // If 1 or 2 balls and StartIndexer switch is tripped, advance indexer to next
    // switch. Ensure ball not already in shooter.

    if ((m_indexer.swIntake.get() == false) && (Indexer.balls == 1) && !m_indexer.isShooterPrimed()) {
      m_indexer.start();
    }
    // Keep Advancing balls until middle switch reached (false is broken beam)
    // if (m_indexer.isRunning() && (Indexer.balls<3) &&
    // m_indexer.swMidIndex.get()==false){
    if (m_indexer.isRunning() && (Indexer.balls == 2) && m_indexer.swMidIndex.get() == false) {
      m_indexer.stop();
    }
    if (m_Intake.isRunning() && (Indexer.balls == 2) && m_indexer.swMidIndex.get() == false) {
      m_Intake.stop();
    }
  ...
  public void end(final boolean interrupted) {
    ...
    if (Constants.kDebug) {
      System.out.print("Magazine CMD END - Intake Stopped with Balls=");
      System.out.println(Indexer.balls);
    }
  }
